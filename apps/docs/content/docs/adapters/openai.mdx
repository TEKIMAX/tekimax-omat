...
- **`dangerouslyAllowBrowser`**: The OpenAI SDK blocks client-side usage by default to prevent API key exposure in browser bundles. Set this to `true` only for prototyping — in production, proxy requests through your backend.
